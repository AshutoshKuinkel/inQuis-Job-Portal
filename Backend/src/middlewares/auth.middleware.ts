...
export const authenticate = (roles?: Role[]) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!inQuis_portal_accessToken) {
      ...
      return next(new CustomError("401 code 5", 401));
    }
    ...
    try {
      ...
      if (Date.now() > decodedData.exp * 1000) {
        res.clearCookie("inQuis_portal_accessToken");
        return next(new CustomError("401 code 6", 401));
      }

      const user = await User.findById(decodedData._id);
      if (!user) {
        return next(new CustomError("401 code 7", 401));
      }

      if (roles && !roles.includes(decodedData.role)) {
        return next(new CustomError("403 code 1", 403));
      }
      ...
      return next(new CustomError("401 code 8", 401));
    }
  };
};
